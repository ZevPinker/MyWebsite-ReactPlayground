fall damage for gravity game 

~~move stop sign and also maybe make it make more sense~~
 
embed links 

somehow include an api thingy to show profficiency

Sign in or contact information and sql database

some kind of back end 

https://a.singlediv.com

https://lynnandtonic.com


professional portfolio and
    a gravity game that if you win you get prompted to give name and email to see my creative portfolio

https://mywebsite-reactplayground-production.up.railway.app/api/submissions

Visitors log viewable here. but data gets cleared every few minutes :(

https://search.google.com/search-console?resource_id=https%3A%2F%2Fzevpinker.com%2F



Put name,

